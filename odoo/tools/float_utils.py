...
def cmp(a, b) -> Literal[-1, 0, 1]:
    return (a > b) - (a < b)


def _float_check_precision(precision_digits: int | None = None, precision_rounding: float | None = None) -> float:
    assert (
        (precision_digits is None) ^ (precision_rounding is None)
    ), "exactly one of precision_digits and precision_rounding must be specified (one must be None and the other not None)"
...
def float_round(
    ...
    deicmal_value = float_to_decimal(value)
    decimal_rounding_factor = float_to_decimal(rounding_factor)
    rounded_value = deicmal_value.quantize(decimal_rounding_factor, rounding=rounding_map[rounding_method])
...
def float_repr(value: float, precision_digits: int):
...
if __name__ == "__main__":
    ...
    def try_round(amount, expected, precision_digits=3):
        ...
        if result != expected:
            errors += 1
            print('###!!! Value %s precision %s Rounding error: got %s , expected %s' % (amount, precision_digits, result, expected))
