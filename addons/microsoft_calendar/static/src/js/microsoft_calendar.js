odoo.define('microsoft_calendar.microsoft_calendar', function (require) {
"use strict";
...
CalendarController.include({
    custom_events: _.extend({}, CalendarController.prototype.custom_events, {
        syncCalendarmicrosoft: '_onSyncCalendarmicrosoft',
    }),
     ...
     */
     _onSyncCalendarmicrosoft: function (event) {
        var self = this;
...
CalendarRenderer.include({
    events: _.extend({}, CalendarRenderer.prototype.events, {
        'click .o_microsoft_sync_button': '_onSyncCalendarmicrosoft',
    }),
    ...
    _initSidebar: function () {
        ...
        if (this.model === "calendar.event") {
            this.$googleButton = $('<button/>', {type: 'button', html: _t("Sync with <b>Microsoft</b>")})
                                .addClass('o_microsoft_sync_button oe_button btn btn-secondary')
                                .prepend($('<img/>', {
                                    src: "/microsoft_calendar/static/src/img/calendar_32.png",
                                }))
     ...
     * @private
     */
     _onSyncCalendarmicrosoft: function () {
        var self = this;
        var context = this.getSession().user_context;
        this.$googleButton.prop('disabled', true);
        this.trigger_up('syncCalendarmicrosoft', {
            on_always: function () {
