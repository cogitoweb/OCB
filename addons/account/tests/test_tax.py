...
class TestTax(AccountTestUsers):
    ...
    def test_tax_group_of_group_tax(self):
        ...
        self.assertEqual(res['base'], 210.0)
        self.assertEqual(res['total_included'], 263.0)

    def test_tax_group(self):
        res = self.group_tax.compute_all(200.0)
        self.assertEqual(res['total_excluded'], 200.0)
        self.assertEqual(res['total_included'], 230.0)
        self.assertEqual(len(res['taxes']), 2)
        self.assertEqual(res['taxes'][0]['amount'], 10.0)
        self.assertEqual(res['taxes'][1]['amount'], 20.0)
    ...
    def test_tax_percent_division(self):
        ...
        self.assertEqual(res_division['taxes'][0]['amount'], 20.0)
        self.assertEqual(res_percent['taxes'][0]['amount'], 20.0)
        ...
        res_division = self.division_tax.compute_all(200.0)
        res_percent = self.percent_tax.compute_all(200.0)
        self.assertEqual(res_division['taxes'][0]['amount'], 22.22)
        self.assertEqual(res_percent['taxes'][0]['amount'], 18.18)
    ...
    def test_tax_sequence_normalized_set(self):
        ...
        self.assertEqual(res['taxes'][0]['amount'], 22.22)
        self.assertEqual(res['taxes'][1]['amount'], 10.0)
        self.assertEqual(res['taxes'][2]['amount'], 20.0)

    def test_tax_include_base_amount(self):
        self.fixed_tax.include_base_amount = True
        res = self.group_tax.compute_all(200.0)
        self.assertEqual(res['total_included'], 231.0)
    ...
    def test_tax_move_lines_creation(self):
        ...
        self.assertEqual(len(aml_fixed_tax), 1)
        self.assertEqual(aml_fixed_tax.credit, 10)
        self.assertEqual(len(aml_percent_tax), 1)
        self.assertEqual(aml_percent_tax.credit, 20)
        self.assertEqual(len(aml_fixed_tax_bis), 1)
        self.assertEqual(aml_fixed_tax_bis.credit, 15)
        
        aml_with_taxes = move.line_ids.filtered(lambda l: set(l.tax_ids.ids) == set([self.group_tax.id, self.fixed_tax_bis.id]))
        self.assertEqual(len(aml_with_taxes), 1)
        self.assertEqual(aml_with_taxes.credit, 190)
