...
class TestSaleExpense(TestSale):
    def test_sale_expense(self):
        ...
        self.assertTrue(prod_exp_1 in [so.product_id for so in so.order_line], 'Sale Expense: expense product should be in so')
        ...
        sheet.action_sheet_move_create()
        # expense should now be in sales order
        self.assertTrue(prod_exp_2 in [so.product_id for so in so.order_line], 'Sale Expense: expense product should be in so')
