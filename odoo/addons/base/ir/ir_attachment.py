...
class IrAttachment(models.Model):
    ...
    def _inverse_datas(self):
        ...
        for attach in self:
            ...
            value = attach.datas
            bin_data = value and base64.b64decode(value) if value else b''
            vals = {
    ...
    def _index(self, bin_data, datas_fname, file_type):
        ...
        if file_type:
            ...
            if index_content == 'text': # compute index_content only for text type
                words = re.findall(b"[^\x00-\x1F\x7F-\xFF]{4,}", bin_data)
                index_content = ustr(b"\n".join(words))
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        for res_model, targets in model_attachments.items():
            ...
            if not self.env[res_model].check_access_rights('read', False):
                ...
                ids.difference_update(itertools.chain(*iter(targets.values())))
                continue
